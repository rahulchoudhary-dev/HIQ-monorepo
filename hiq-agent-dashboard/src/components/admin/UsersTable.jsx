...
import { getUserRole, getInitial } from "@/utils/userUtils";
              ...
              <tr key={user.id} className="hover:bg-muted/30 transition">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex flex-wrap gap-1">
                    {getUserSites(user.auth_user_id, userSites).length === 0 ? (
                      <span className="text-xs text-muted-foreground">
                        No sites assigned
                      </span>
                    ) : (
                      getUserSites(user.auth_user_id, userSites).map(
                        (userSite) => (
                          <span
