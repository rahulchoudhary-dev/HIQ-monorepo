...
const AGENT_OPTIONS = [
  ...
  'EPA rule book',
  'Recycling Hunter',
  'Reuse Fill pathway', // <- New option for agent
]

export default function BugReport({ selectedAgent, defaultFileName = '', onClose }) {
  const [sir, setSir] = useState(''); // NEW: site (sir) field
  ...
  const invalid = useMemo(() => {
    if (!sir) return 'Please select a site.' // Require site
    ...
    return null
  }, [sir, agentName, expected, actual, desc])

  const submit = async () => {
    setSending(true); setStatus(null)
    try {
      if (!sir) throw new Error('Please select a site.') // Require site
      if (invalid) throw new Error(invalid)

      const payload = {
        site: sir, // NEW: send site as sir field
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-1">
                ...
                {!sir && <p className="mt-1 text-xs text-gray-400">Choose the relevant site</p>}
              </div>
              {/* Agent Name */}
              <div className="md:col-span-1">
